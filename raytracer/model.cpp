...
Model::Model(const std::string& modelPath)
{
	...
	for (const auto& shape : obj.shapes)
	{
		...
		for (const auto& index: shape.mesh.indices)
		{
			...
			if ((count % 3) == 0)
			{
				mTriangles.emplace_back(temp[0], temp[1], temp[2]);
			}
		}
	}
}
...
bool Model::hit(const ray& r, interval ray_t, hit_record& rec) const
{
	hit_record tempHR;
	rec.t = kDoubleMax;
	bool hit = false;
...
{
	vec3 min{ kDoubleMin, kDoubleMin, kDoubleMin };
	vec3 max{ kDoubleMax, kDoubleMax, kDoubleMax };
